...
.. image:: /images/minio-console.png
   :width: 600px
...
Each bucket has a :guilabel:`Manage` and :guilabel:`Browse` button.

- Select :guilabel:`Manage` to manage the following bucket configuration settings:

- The :guilabel:`Summary` view displays a summary of the bucket configuration.
...
.. tab-set::
   ...
   .. tab-item:: Users
      ...
      For deployments using MinIO as the identity manager, this section is only visible if the authenticated user does has the :ref:`required administrative permissions <minio-policy-mc-admin-actions>`.
   ...
   .. tab-item:: Service Accounts
      ...
      This section is only visible if the authenticated user does has the :ref:`required administrative permissions <minio-policy-mc-admin-actions>`.
...
- Select the policy row to manage the policy details.
  ...
  The :guilabel:`Users` view displays all users assigned the policy.

  The :guilabel:`Groups` view displays all groups assigned the policy.
...
You can also register your deployment with MinIO |subnet| to allow upload of health reports directly through the MinIO Console.
...
.. tab-set::
   ...
   .. tab-item:: Register
      ...
      The :guilabel:`Register` section provides a login form for connecting your MinIO deployment to |subnet|.

      You can then upload your deployment health reports directly to SUBNET for reference by MinIO Engineering.
   ...
   .. tab-item:: Profile
      ...
      The results can provide insight into the MinIO server process per-node.
   ...
   .. tab-item:: Inspect
      ...
      The resulting object may be read using MinIO's :minio-git:`debugging tool <minio/tree/master/docs/debugging#decoding-metadata>`. Independent or third-party use of the output for diagnostics or remediation is done at your own risk.
...
.. tab-set::
   ...
   .. tab-item:: Notifications
      ...
      You can select an existing notification target from the list to view it's details.
   ...
   .. tab-item:: Tiers
      ...
      You can select an existing tier from the list to view it's details.
